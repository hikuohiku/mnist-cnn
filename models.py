...
class Complex_CNN(nn.Module):
    def __init__(self):
        ...
        self.conv_layers = nn.Sequential(
            ...
            nn.ReLU(),
            nn.MaxPool2d(kernel_size=2, stride=2),  # プーリング層2
            nn.Conv2d(64, 128, kernel_size=3, stride=1, padding=1),  # 畳み込み層3
            nn.ReLU(),
            nn.MaxPool2d(kernel_size=2, stride=2),  # プーリング層3
        )
        self.fc_layers = nn.Sequential(
            nn.Linear(128 * 3 * 3, 128),  # 全結合層1
            nn.ReLU(),
            nn.Linear(128, 10),  # 全結合層2（出力層）
        )
...
class Simple_CNN2(nn.Module):
    def __init__(self):
        super().__init__()
        self.conv_layers = nn.Sequential(
            nn.Conv2d(1, 64, kernel_size=3, stride=1, padding=1),  # 畳み込み層1
            nn.ReLU(),
            nn.MaxPool2d(kernel_size=4, stride=4),  # プーリング層1
        )
        self.fc_layers = nn.Sequential(
            nn.Linear(64 * 7 * 7, 128),  # 全結合層1
            nn.ReLU(),
